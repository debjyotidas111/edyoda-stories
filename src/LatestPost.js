import React from 'react'
import './LatestPost.css';


const LatestPost = () => {
  return (
    <>
    <main>
      
      <h1 className="trending-post">Latest Post</h1>

<div className="Categories-container">

  <div className="Heading">
    <img src='data:image/png;base64,' alt='icon'></img>
    {/* <i className="fa-sharp fa-solid fa-filter fa-lg" style={{color:" #0084ff; "}}></i> */}
    <span>Filter by Category</span>
  </div>

  <div className="Categories">

  <button className="btn-category active">All</button>
    <button className="btn-category">Artificial Intelligence</button>
    <button className="btn-category">Cloud Computing</button>
    <button className="btn-category">DevOps</button>
    <button className="btn-category">Programming Languages</button>
    <button className="btn-category">Mobile Application Development</button>
    <button className="btn-category">Technology & Tools</button>
    <button className="btn-category">Get A Job In A Tech Company</button>
    <button className="btn-category">Others</button>
    
  </div>

</div>
 
</main>

    </>
  )
}

export default LatestPost
